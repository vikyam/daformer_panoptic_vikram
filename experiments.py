...
def get_model_base(architecture, backbone):
    ...
    return {
        ...
        'dlv2pan': '_base_/models/deeplabv2pan_r50-d8.py',
        'dlv2pan2': '_base_/models/deeplabv2pan2_r50-d8.py',
        'dlv2red': '_base_/models/deeplabv2red_r50-d8.py',
        'dlv2redpan': '_base_/models/deeplabv2redpan_r50-d8.py',
...
def generate_experiment_cfgs(id):
    ...
    def config_from_vars():
        cfg = {'_base_': ['_base_/default_runtime.py'], 'n_gpus': n_gpus, 'gpu_mtotal': gpu_mtotal}
        if seed is not None:
            cfg['seed'] = seed
        ...
        # for dict(lr_mult=10.) refer to: https://mmsegmentation.readthedocs.io/en/latest/tutorials/training_tricks.html TODO
        if pmult and architecture != 'dlv2pan':
            opt_param_cfg['head'] = dict(lr_mult=10.)
        elif pmult and architecture == 'dlv2pan':
            opt_param_cfg['semanitc_head'] = dict(lr_mult=10.)
        elif pmult and architecture == 'dlv2pan2':
            opt_param_cfg['mtl_block'] = dict(lr_mult=10.)
            opt_param_cfg['semanitc_head'] = dict(lr_mult=10.)
            opt_param_cfg['instance_head'] = dict(lr_mult=10.)
            opt_param_cfg['center_sub_head'] = dict(lr_mult=10.)
            opt_param_cfg['offset_sub_head'] = dict(lr_mult=10.)
        else:
            raise NotImplementedError(f"Not implemented for pmult = {pmult}")
    ...
    # ---------------------------------------------------------------------------------------------
    # Reproducing DACS using my panoptic dataloader
    # ---------------------------------------------------------------------------------------------
    if id == 1001:
        gpu_mtotal = 10240
        iters = 250000
        opt, lr, schedule, pmult = 'sgddapnet', 2.5e-4, 'poly10dapnet', True
        seeds = [0, 1, 2]
        datasets = [('synthia', 'cityscapes')]
        models = [('dlv2pan', 'r101v1c')]
        udas = ['dacs_panoptic']
        for (source, target), (architecture, backbone), uda, seed in itertools.product(datasets, models, udas, seeds):
            cfg = config_from_vars()
            cfg['evaluation'] = dict(
                interval=5000,
                # interval=iters // 10, # Original
                metric='mIoU',
                eval_type='daformer',
                ...
                gt_dir='',
                debug=False,
                num_samples_debug=12, # there are maximum 12 val images in the validation debug set
                gt_dir_panop=''
            )
            cfg['activate_panoptic'] = False
            cfgs.append(cfg)

    # ---------------------------------------------------------------------------------------------
    # Reproducing DAFormer using my panoptic dataloader
    # ---------------------------------------------------------------------------------------------
    elif id == 1002:
        gpu_mtotal = 10240
        # gpu_mtotal = 23000
        ...
        uda = 'dacs_panoptic_a999_fdthings'
        rcs_T = 0.01
        plcrop = True
    ...
    # ---------------------------------------------------------------------------------------------
        ...
        seeds = [0, 1, 2]
        datasets = [('synthia', 'cityscapes')]
        models = [('dlv2pan2', 'r101v1c')]
        udas = ['dacs_panoptic']
        for (source, target), (architecture, backbone), uda, seed in itertools.product(datasets, models, udas, seeds):
            cfg = config_from_vars()
            cfg['evaluation'] = dict(
                interval=30000, # 5000: TODO
                # interval=iters // 10, # Original
                metric='mIoU',
                # eval_type='daformer',
                eval_type='panop_deeplab',
            ...
            cfg['activate_panoptic'] = True
            cfgs.append(cfg)

    # ---------------------------------------------------------------------------------------------
    # Panoptic DAFormer : instance head is added to the daformer_head
    # ---------------------------------------------------------------------------------------------
    elif id == 1004:
        # iters = 40000
        # gpu_mtotal = 10240
        ...
        for (source, target), seed in \
            ...
            cfg['activate_panoptic'] = True
            cfg['data']['samples_per_gpu'] = 2  # TODO
            cfgs.append(cfg)

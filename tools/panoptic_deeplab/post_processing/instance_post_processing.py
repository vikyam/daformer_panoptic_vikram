...
def find_instance_center(ctr_hmp, threshold=0.1, nms_kernel=3, top_k=None):
    ...
    ctr_hmp = ctr_hmp.squeeze() # ctr_hmp: tensor(1024,2048) # TODO: this is only applicable for single batch, we need to do it for batch size 2
...
def get_instance_segmentation(sem_seg, ctr_hmp, offsets, thing_list, threshold=0.1, nms_kernel=3, top_k=None,
    ...
    ctr = find_instance_center(ctr_hmp, threshold=threshold, nms_kernel=nms_kernel, top_k=top_k) # ctr:tensor:(N,2), N is the no of center
    if ctr.size(0) == 0:
...
def get_panoptic_segmentation(sem, ctr_hmp, offsets, thing_list, label_divisor, stuff_area, void_label,
    ...
    instance, center = get_instance_segmentation(semantic, ctr_hmp, offsets, thing_list, threshold=threshold, nms_kernel=nms_kernel, top_k=top_k, thing_seg=thing_seg)

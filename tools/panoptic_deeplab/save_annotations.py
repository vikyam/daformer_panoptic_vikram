...
def save_annotation(label,
    ...
    # if dataset_name in 'Cityscapes':
    #     str1 = filename.split('/')
    #     sub_folder_name = str1[0]
    #     file_name = str1[1]
    #     # out_path = save_dir
    #     out_path = os.path.join(save_dir, sub_folder_name)
    #     if not os.path.exists(out_path):
    #         os.makedirs(out_path)
    # elif dataset_name in 'Mapillary':
    #     raise NotImplementedError(f'No implementation found for dataset {dataset_name}')

    out_fname = '%s/%s.png' % (save_dir, filename)

    with open(out_fname, mode='wb') as f:
        pil_image.save(f, 'PNG')
        if debug:
            logger.info('File saved at: {}'.format(out_fname))
...
def save_instance_annotation(label,
    ...
    with open(out_fname, mode='wb') as f:
        pil_image.save(f, 'PNG')
        if DEBUG:
            print('file saved at: {}'.format(out_fname))


def save_panoptic_annotation(label, save_dir, filename, label_divisor, colormap=None, image=None, blend_ratio=0.5):
    """Saves the given label to image on disk.
    Args:
        label: The numpy array to be saved. The data will be converted
            to uint8 and saved as png image.
...
def save_heatmap_image(image,
    ...
    with open(out_fname, mode='wb') as f:
        pil_image.save(f, 'PNG')
    if DEBUG:
        print('Heat map image is saved at: {}'.format(out_fname))
...
def save_offset_image_v2(image, offset, save_dir, filename, ratio=0.5, DEBUG=False):
    offset_image = flow_compute_color(offset[:, :, 1], offset[:, :, 0])
    ...
    with open(out_fname, mode='wb') as f:
        pil_image.save(f, 'PNG')
        if DEBUG:
            print('offset map is saved at: {}'.format(out_fname))
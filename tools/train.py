...
def main(args):
    ...
    # logger.info(model) # TODO: I don't want to print the model now in the logger

    logger.info('#############################################################')
    logger.info('cfg[log_config][interval]={}'.format(cfg['log_config']['interval']))
    logger.info('cfg[evaluation][num_samples_debug]={}'.format(cfg['evaluation']['num_samples_debug']))
    logger.info('cfg[evaluation][interval]={}'.format(cfg['evaluation']['interval']))
    logger.info('cfg[uda][debug_img_interval]={}'.format(cfg['uda']['debug_img_interval']))
    logger.info('cfg[checkpoint_config][interval]={}'.format(cfg['checkpoint_config']['interval']))
    logger.info('cfg[data][workers_per_gpu]={}'.format(cfg['data']['workers_per_gpu']))
    logger.info('cfg[data][samples_per_gpu]={}'.format(cfg['data']['samples_per_gpu']))
    logger.info('cfg[model][decode_head][loss_decode][1][loss_weight]={}'.format(cfg['model']['decode_head']['loss_decode'][1]['loss_weight']))
    logger.info('#############################################################')
    ...
    train_segmentor(
        ...
        validate=(not args.no_validate), # TODO
        timestamp=timestamp,
        meta=meta)

...
# Determine the labels that have instances # TODO: original code
...
def readGTImage(gtImageFileName, args, mapillary_dataloading_style='OURS',
    ...
    if 'Mapillary' in dataset_name:
        if mapillary_dataloading_style == 'DADA':
            ...
            # imgNp = rgb2id(imgNp)
            # img = Image.fromarray(imgNp) # converting back to PIL as the caller function wants this as PIL image
        else:
...
def getGtInstances(groundTruthList, args, dataset_name=None, rgb2id=None, input_image_size=None, mapillary_dataloading_style='OURS', logger=None):
    gtInstances = {}
    ...
    # otherwise create it
        ...
        gtInstances = instances2dict(groundTruthList,not args.quiet, dataset_name=dataset_name, rgb2id=rgb2id, input_image_size=input_image_size, mapillary_dataloading_style=mapillary_dataloading_style)
...
def matchGtWithPreds(predictionList, groundTruthList, gtInstances, args, mapillary_dataloading_style='OURS',
                     dataset_name=None, rgb2id=None, input_image_size=None, logger=None, debug=False):
    matches = {}
    if not args.quiet:
        logger.info("Matching {} pairs of images...".format(len(predictionList)))
    ...
    for (pred, gt) in zip(predictionList, groundTruthList):
        ...
        # Read input files
        gtImage  = readGTImage(gt, args, mapillary_dataloading_style=mapillary_dataloading_style,
                               dataset_name=dataset_name, rgb2id=rgb2id,
                               input_image_size=input_image_size, debug=debug, logger=logger) # TODO: updated for adatping to mapillary evaluation
...
def evaluateImgLists(predictionList, groundTruthList, args, dataset_name=None,
                     rgb2id=None, input_image_size=None,

    ...
    for gt_ann, pred_ann in annotation_set:
        ...
        if 'Mapillary' in dataset_name:
            ...
            if mapillary_dataloading_style == 'DADA':
                # pan_gt, _ = resize_with_pad(input_image_size, pan_gt, Image.NEAREST, fill_value=0, is_label=True)
                # pan_gt = pad_with_fixed_AS(input_image_size[0] / input_image_size[1], pan_gt, fill_value=0, is_label=True)
            ...
            #     pan_gt = pan_gt.resize(size_new, Image.NEAREST)
            #     pan_gt = np.asarray(pan_gt, dtype=np.uint32)
            #     # pan_gt = pan_gt.transpose((1, 0, 2))
            #     pan_gt = rgb2id(pan_gt)
            else:
                 NotImplementedError('No implementation Error --> cityscapesscripts/evaluation/evalPanopticSemanticLabeling.py')
...
def evaluatePanoptic(gt_json_file, gt_folder, pred_json_file, pred_folder,
                     resultsFile, debug=None, dataset_name=None,
    ...
    pq_stat = pq_compute_multi_core(matched_annotations_list, gt_folder,
                                    pred_folder, categories, debug, dataset_name,
                                    input_image_size, mapillary_dataloading_style, logger)

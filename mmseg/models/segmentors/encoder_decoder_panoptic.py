...
class EncoderDecoderPanoptic(BaseSegmentorPanoptic):
    ...
    def __init__(self,
        ...
        super(EncoderDecoderPanoptic, self).__init__(init_cfg)
    ...
    def whole_inference(self, img, img_meta, rescale, debug, eval_type):
        """Inference with full image."""

        # print(f'torch.onnx.is_in_onnx_export(): {torch.onnx.is_in_onnx_export()}') # TODO:comment
    ...
    def inference(self, img, img_meta, rescale, eval_type, debug):
        ...
        if self.test_cfg.mode == 'slide':
            ...
            if not self.activate_panoptic:
                semantic_logit = self.whole_inference(img, img_meta, rescale, debug, eval_type)
            else:
                semantic_logit, center_logit, offset_logit = self.whole_inference(img, img_meta, rescale, debug, eval_type)
    ...
    def simple_test(self, img, img_meta, rescale=True, **kwargs):
        ...
        if not self.activate_panoptic:
            semantic_logit = self.inference(img, img_meta, rescale, eval_type, debug)
        else:
            semantic_logit, center_logit, offset_logit = self.inference(img, img_meta, rescale, eval_type, debug)
        ...
        pred = {}
        if not self.activate_panoptic:
            pred['semantic'] = semantic_logit
        else:
            pred['semantic'] = semantic_logit
            pred['center'] = center_logit
            pred['offset'] = offset_logit
        return pred

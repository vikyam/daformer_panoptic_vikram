...
class BaseDecodeHeadPanoptic(BaseModule, metaclass=ABCMeta):
    ...
    def forward_train(self,
        ...
        if not self.act_panop:
            ...
            if img_metas:
                ...
                losses = self.losses_panoptic(semantic_logits, gt_semantic_seg, seg_weight,
    ...
    def losses_panoptic(self, seg_logit, seg_label, seg_weight=None, **kwargs):
        ...
        if avg_factor > 0:
            loss['loss_center'] = self.loss_decode[1](center_logits, gt_center.squeeze(dim=1), weight=center_weights, avg_factor=avg_factor)
        elif avg_factor == 0:
            avg_factor = 1
            loss['loss_center'] = self.loss_decode[1](center_logits, gt_center.squeeze(dim=1), weight=center_weights, avg_factor=avg_factor)
        else:
        ...
        if avg_factor > 0:
            loss['loss_offset'] = self.loss_decode[2](offset_logits, gt_offset.squeeze(dim=1), weight=offset_weights, avg_factor=avg_factor)
        elif avg_factor == 0:
            avg_factor = 1
            loss['loss_offset'] = self.loss_decode[2](offset_logits, gt_offset.squeeze(dim=1), weight=offset_weights, avg_factor=avg_factor)
        else:

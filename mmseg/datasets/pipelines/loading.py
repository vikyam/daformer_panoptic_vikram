...
class LoadPanopticAnnotations(object):
    ...
    def __call__(self, results):
        ...
        # Original code
        # if False: # original code block - this is not working for panoptic gt pngs, when i use rgb2id on gt_panoptic_seg then I get very large numbers instead of  getting instance ids like 24000 24001, might  due to bgr and rgb setting not sure
        #     img_bytes = self.file_client.get(filename)
        ...
        # if results.get('label_map', None) is not None:
        #     for old_id, new_id in results['label_map'].items():
        #         gt_panoptic_seg[gt_panoptic_seg == old_id] = new_id
        # reduce zero_label
        # if self.reduce_zero_label:
        #     # avoid using underflow conversion
        #     gt_panoptic_seg[gt_panoptic_seg == 0] = 255
        #     gt_panoptic_seg = gt_panoptic_seg - 1
        #     gt_panoptic_seg[gt_panoptic_seg == 254] = 255
